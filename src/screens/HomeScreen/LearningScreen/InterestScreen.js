...
import { saveListSubjectInterest, saveSubjectIdTouched } from '../../../redux/actions/subject';
    ...
    const renderItem = ({ item }) => {
        return (
            <WingBlank size='sm' style={styles.container}>
                {item.listSubjects.length > 0 && <View key={item.topicDetail.topicId} style={styles.item}>
                    <View>
                        {item.listSubjects.length > 0 &&
                            <Text style={styles.subjectTitle} >#{item.topicDetail.topicName}</Text>}
                        {item.listSubjects &&
                                        ...
                                        <Card style={styles.card} >
                                            ...
                                            <Card.Body>
                                                <View style={{ minHeight: 20 }}>
                                                    <Text style={{ marginLeft: 16 }}>{subject.subjectDescription}</Text>
                                                    <Text style={{ marginLeft: 16, color: 'blue' }}>{subject.author}</Text>
